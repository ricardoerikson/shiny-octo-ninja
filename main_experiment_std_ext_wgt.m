	...
	for j=1:max(size(nn_size))
		parfor k=1:max(size(test_data))
			...
			if(min(size(knn))>0)
				...
				prediction_ext(k,i,j) = predict_rating(ext_r_ubar,ext_r_vi,ext_r_vbar,ext_sim_uv);
				prediction_ext_wgt(k,i,j) = predict_rating_weighted(ext_r_ubar,ext_r_vi,ext_r_vbar,ext_sim_uv,ex_weight);
			end;
		end;
	end;
end;
